...
from apt.utils.datasets import DATA_PANDAS_NUMPY_TYPE, ArrayDataset
from apt.utils.models import (
...
class GeneralizeToRepresentative(BaseEstimator, MetaEstimatorMixin, TransformerMixin):
    ...
    def __init__(
        self,
        estimator: Union[BaseEstimator, Model] = None,
        ...
        if estimator is not None and not issubclass(estimator.__class__, Model):
            if is_regression:
                ...
                self.estimator = SklearnClassifier(
                    estimator, CLASSIFIER_SINGLE_OUTPUT_CLASS_PROBABILITIES
                )
        ...
        if self.feature_slices:
            self.all_one_hot_features = {
                str(feature) for encoded in self.feature_slices for feature in encoded
            }
        else:
    ...
    def set_params(self, **params):
        ...
        if "train_only_features_to_minimize" in params:
            self.train_only_features_to_minimize = params[
                "train_only_features_to_minimize"
            ]
    ...
    def fit_transform(
        self,
        X: Optional[DATA_PANDAS_NUMPY_TYPE] = None,
        y: Optional[DATA_PANDAS_NUMPY_TYPE] = None,
        ...
        if not self.generalize_using_transform:
            raise ValueError(
                "fit_transform method called even though generalize_using_transform parameter was False. "
    ...
    def fit(
        ...
        if (
            dataset
            and dataset.get_samples() is not None
            and dataset.get_labels() is not None
        ):
        ...
        if (
            ...
            if not all(elem in self._features for elem in self.features_to_minimize):
                raise ValueError(
                    "features to minimize should be a subset of features names"
                )
            if self.feature_slices:
                temp_list = []
                for slice in self.feature_slices:
                    ...
                    if not all(elem in self._features for elem in new_slice):
                        raise ValueError(
                            "features in slices should be a subset of features names"
                        )
            ...
            if self.is_regression:
                x_train, x_test, y_train, y_test = train_test_split(
                    x, dataset.get_labels(), test_size=0.4, random_state=14
                )
            else:
                try:
                    x_train, x_test, y_train, y_test = train_test_split(
                    ...
                    x_train, x_test, y_train, y_test = train_test_split(
                        x, dataset.get_labels(), test_size=0.4, random_state=18
                    )
            ...
            # CONTRIBUTION: Fix the "UserWarning: X does not have valid feature names, but DecisionTreeClassifier was fitted with feature names"
            # In the original code it's caused by the fact that we fit the internal DT with a named pandas dataframe
            # however, during the DT.decision_path() function call, the data is internally converted to a numpy array without feature names,
            # which causes the warning.
            #
            # To fix this, we convert to numpy to avoid feature name warnings with internal decision tree
            # The DT doesn't need feature names since we track them separately in self._encoded_features
            self._dt.fit(x_prepared.to_numpy(), y_train)
            ...
            for feature in self._features:
                if feature not in self.features_to_minimize:
                    self._remove_feature_from_cells(
                        self.cells, self._cells_by_id, feature
                    )
            ...
            accuracy = self._calculate_accuracy(
                generalized, y_test, self.estimator, self.encoder
            )
            print(
                "Initial accuracy of model on generalized data, relative to original model predictions "
                "(base generalization derived from tree, before improvements): %f"
                % accuracy
            )
            ...
            if accuracy > self.target_accuracy:
                ...
                while accuracy > self.target_accuracy:
                    ...
                    self._attach_cells_representatives(
                        x_prepared, used_x_train, y_train, nodes
                    )
                    ...
                    if accuracy < self.target_accuracy:
                        ...
                        print(
                            "Pruned tree to level: %d, new relative accuracy: %f"
                            % (self._level, accuracy)
                        )
            ...
            # if accuracy below threshold, improve accuracy by removing features from generalization
                ...
                while accuracy < self.target_accuracy:
                    ...
                    print(
                        "Removed feature: %s, new relative accuracy: %f"
    ...
    def transform(
        ...
        if not self.generalize_using_transform:
            raise ValueError(
                "transform method called even though generalize_using_transform parameter was False. This "
    ...
    def calculate_ncp(self, samples: ArrayDataset):
        ...
        if self.generalize_using_transform:
            ...
            counted = np.zeros(
                samples_pd.shape[0]
            )  # to mark records we already counted
            ncp = 0
            for cell in self.cells:
                ...
                ncp += self._calc_ncp_for_generalization(
                    generalizations[cell["id"]],
                    range_counts,
                    category_counts,
                    total_samples,
                )
            ...
            range_counts = self._find_range_counts(
                samples_pd, generalizations["ranges"]
            )
            category_counts = self._find_category_counts(
                samples_pd, generalizations["categories"]
            )
            ncp = self._calc_ncp_for_generalization(
                generalizations, range_counts, category_counts, total_samples
            )

        return ncp

    def _inner_transform(
        self,
        x: Optional[DATA_PANDAS_NUMPY_TYPE] = None,
        features_names: Optional[list] = None,
        dataset: Optional[ArrayDataset] = None,
        ...
        msg = (
            "This %(name)s instance is not initialized yet. "
        ...
        if (
            self._dt
        ):  # only works if fit was called previously (but much more efficient)
            nodes = self._get_nodes_level(self._level)
            ...
            generalized = self._generalize_from_tree(
                x_pd, prepared, nodes, self.cells, self._cells_by_id
            )
        else:
    ...
    def _calc_ncp_categorical(categories, category_count, feature_data, total):
        ...
        normalized_category_sizes = [
            s * n / total for s, n in zip(category_sizes, category_count)
        ]
        average_group_size = sum(normalized_category_sizes) / len(
            normalized_category_sizes
        )
        return (
            average_group_size / feature_data["range"]
        ...
        for feature in self._features:
            if feature not in feature_data.keys():
                ...
                if (
                    ...
                    and feature not in self.all_one_hot_features
                ):
                    fd["min"] = min(values)
                    fd["max"] = max(values)
    ...
    def _cell_contains(self, cell, row, index, mapped):
        for i, feature in enumerate(self._features):
            if feature in cell["ranges"]:
                ...
                if not self._cell_contains_categorical(
                    i, cell["categories"][feature], row
                ):
                    return False
                ...
                raise TypeError(
                    "feature " + str(feature) + " not found in cell " + str(cell["id"])
                )

        # Mark as mapped
        # CONTRIBUTION: since numpy 2.0 ndarray.itemset is deprecated
        # using direct indexing instaed
            ...
            if feature in used_features:
                try:
                    ...
                    x[feature] = pd.Categorical(
                        x.loc[:, feature],
                        categories=self._categorical_values[feature],
                        ordered=False,
                    )
    ...
    def _calculate_cell_label(self, node):
        ...
        if isinstance(self._dt.classes_, list):
            return [
                self._dt.classes_[output][class_index]
        ...
        for cell in self.cells:
            ...
            for feature in features:
                if feature in self._one_hot_vector_features_to_features.keys():
                    ...
                    categorical_feature = self._one_hot_vector_features_to_features[
                        feature
                    ]
                    if categorical_feature not in new_cell["categories"].keys():
                        new_cell["categories"][categorical_feature] = (
                            self._categorical_values[categorical_feature].copy()
                        )
                    if feature in cell["ranges"].keys():
                        ...
                        if cell["ranges"][feature]["start"] is not None:
                            ...
                            if (
                                categorical_value
                                in new_cell["categories"][categorical_feature]
                            ):
                                new_cell["categories"][categorical_feature].remove(
                                    categorical_value
                                )
                # features that were already one-hot encoded. Legal values should be 0 or 1
                    ...
                    if feature in cell["ranges"]:
                        ...
                        other_features, encoded = self._get_other_features_in_encoding(
                            feature, self.feature_slices
                        )
                        for other_feature in other_features:
                            if feature_value == 1:
                                ...
                                other_feature not in new_cell["categories"].keys()
                ...
                for node in nodes:
                    if self._dt.tree_.feature[node] == -2:  # leaf node
                        ...
                        for feature in left_cell["ranges"].keys():
                            ...
                            new_cell["ranges"][feature]["start"] = left_cell["ranges"][
                                feature
                            ]["start"]
                            new_cell["ranges"][feature]["end"] = right_cell["ranges"][
                                feature
                            ]["start"]
                        for feature in left_cell["categories"].keys():
                            new_cell["categories"][feature] = list(
                                set(left_cell["categories"][feature])
                        ...
                        self._calculate_level_cell_label(
                            left_cell, right_cell, new_cell
                        )
    ...
    def _calculate_level_cell_label(self, left_cell, right_cell, new_cell):
        ...
        if isinstance(self._dt.classes_, list):
            new_cell["label"] = [
                self._dt.classes_[output][class_index]
                for output, class_index in enumerate(
                    np.argmax(new_cell["hist"], axis=1)
                )
            ]
        else:
    ...
    def _get_nodes_level(self, level):
        ...
        while len(stack) > 0:
            ...
            if (
                self._dt.tree_.children_left[node_id]
                != self._dt.tree_.children_right[node_id]
            ):
        ...
        return [
            i
            for i, x in enumerate(node_depth)
            if x == depth or (x < depth and is_leaves[i])
        ]

    def _attach_cells_representatives(
        self, prepared_data, original_train_features, label_feature, level_nodes
    ):
        ...
        for cell in self.cells:
            ...
            if self.is_regression or (
                len(label_feature.shape) > 1 and label_feature.shape[1] > 1
            ):
    ...
    def _find_sample_nodes(self, samples, nodes):
        # Convert DataFrame to numpy array to match the format used during fit
        samples_array = (
            samples.to_numpy() if isinstance(samples, pd.DataFrame) else samples
        )
        paths = self._dt.decision_path(samples_array).toarray()
        nodeSet = set(nodes)
        return [
            (list(set([i for i, v in enumerate(p) if v == 1]) & nodeSet))[0]
            for p in paths
        ]
    ...
    def _generalize_from_generalizations(self, original_data, generalizations):
        sample_indexes = self._map_to_ranges_categories(
            original_data, generalizations["ranges"], generalizations["categories"]
        )
        original_data_generalized = pd.DataFrame(
            original_data, columns=self._features, copy=True
        )
        for feature in self._generalizations["categories"]:
            if (
                "untouched" not in generalizations
                or feature not in generalizations["untouched"]
            ):
                for g_index, group in enumerate(generalizations["categories"][feature]):
                    indexes = [
                        i for i, s in enumerate(sample_indexes) if s[feature] == g_index
                    ]
                    if indexes:
                        rows = original_data_generalized.iloc[indexes]
                        rows[feature] = generalizations["category_representatives"][
                            feature
                        ][g_index]
        for feature in self._generalizations["ranges"]:
            if (
                ...
                for r_index, range in enumerate(generalizations["ranges"][feature]):
                    indexes = [
                        i for i, s in enumerate(sample_indexes) if s[feature] == r_index
                    ]
                    if indexes:
                        rows = original_data_generalized.iloc[indexes]
                        rows[feature] = generalizations["range_representatives"][
                            feature
                        ][r_index]
        return original_data_generalized

    def _generalize_from_tree(
        self, original_data, prepared_data, level_nodes, cells, cells_by_id
    ):
        ...
        for i in range(len(cells)):
            ...
            for feature in self._features:
                ...
                if feature in cells[i]["representative"] and (
                    "untouched" not in cells[i] or feature not in cells[i]["untouched"]
                ):
                    representatives.loc[i, feature] = cells[i]["representative"][
                        feature
                    ]
            ...
            if indexes and not representatives.columns.empty:
                if len(indexes) > 1:
                    replace = pd.concat(
                        [representatives.loc[i].to_frame().T] * len(indexes)
                ...
                original_data_generalized.loc[
                    indexes, representatives.columns.tolist()
                ] = replace
    ...
    def _generalize(self, data, data_prepared, nodes):
        ...
        if self.generalize_using_transform:
            generalized = self._generalize_from_tree(
                data, data_prepared, nodes, self.cells, self._cells_by_id
            )
        else:
            generalized = self._generalize_from_generalizations(
                data, self.generalizations
            )
        return generalized
    ...
    def _remove_feature_from_generalization(
        self,
        original_data,
        prepared_data,
        nodes,
        ...
        # prepared data include one hot encoded categorical data,
        # if there is no categorical data prepared data is original data
        feature = self._get_feature_to_remove(
            original_data,
            prepared_data,
            nodes,
            labels,
    ...
    def _get_feature_to_remove(
        ...
        print(
            "feature to remove: "
            + (str(remove_feature) if remove_feature is not None else "none")
        )
    ...
    def _normalize_ncp_by_accuracy_gain(
        self,
        original_data,
        prepared_data,
        nodes,
        ...
        generalized = self._generalize_from_tree(
            original_data, prepared_data, nodes, new_cells, cells_by_id
        )
        accuracy = self._calculate_accuracy(
            generalized, labels, self.estimator, self.encoder
        )
        ...
        if samples is not None:
            ...
            for feature in self._generalizations["categories"]:
                if feature not in done:
                    ...
                    for g_index, group in enumerate(
                        self._generalizations["categories"][feature]
                    ):
                        indexes = [
                            i
                            for i, s in enumerate(sample_indexes)
                            if s[feature] == g_index
                        ]
                        if indexes:
                            rows = samples.iloc[indexes]
                            if feature in self.all_one_hot_features:
                                other_features, encoded = (
                                    self._get_other_features_in_encoding(
                                        feature, self.feature_slices
                                    )
                                )
                                values = rows.loc[:, encoded].to_numpy()
                                unique_rows, counts = np.unique(
                                    values, axis=0, return_counts=True
                                )
                            ...
                            category_representatives[feature].append(
                                old_category_representatives[feature][g_index]
                            )
            ...
            for feature in self._generalizations["ranges"]:
                ...
                for index in range(len(self._generalizations["ranges"][feature])):
                    indexes = [
                        i for i, s in enumerate(sample_indexes) if s[feature] == index
                    ]
                    if indexes:
                        rows = samples.iloc[indexes]
                        values = rows[feature]
                        ...
                        range_representatives[feature].append(
                            old_range_representatives[feature][index]
                        )
    ...
    def _calculate_cell_generalizations(self):
        ...
        for cell in self.cells:
            cell_generalizations[cell["id"]] = self._calculate_generalizations_for_cell(
                cell
            )
    ...
    def _calculate_categories(self, cells):
        ...
        categorical_features_values = (
            GeneralizeToRepresentative._calculate_categorical_features_values(cells)
        )
            ...
            for i in range(len(values)):
                ...
                for j in range(len(values)):
                    if j <= i:
                        continue
                    value2 = values[j]
                    if GeneralizeToRepresentative._are_inseparable(
                        cells, feature, value1, value2
                    ):
                ...
                if len(partition) > 1 and feature in self.all_one_hot_features:
                    other_features, _ = self._get_other_features_in_encoding(
                        feature, self.feature_slices
                    )
                    assigned = False
                    for other_feature in other_features:
    ...
    def _calculate_categorical_features_values(cells):
        ...
        for cell in cells:
            for feature in [
                key
                for key in cell["categories"].keys()
                if "untouched" not in cell or key not in cell["untouched"]
            ]:
    ...
    def _remove_feature_from_cells(self, cells, cells_by_id, feature):
        if feature in self.all_one_hot_features:
            for encoded in self.feature_slices:
                if feature in encoded:
                    self._remove_feature_from_cells_internal(
                        cells, cells_by_id, encoded
                    )
        else:
    ...
    def _remove_categorical_untouched(generalizations):
        ...
        for feature in generalizations["categories"].keys():
            category_sizes = [
                len(g) if len(g) > 1 else 0
                for g in generalizations["categories"][feature]
            ]
    ...
    def _calculate_accuracy(generalized, y_test, estimator, encoder):
        ...
        feature_names = (
            encoder.get_feature_names_out() if encoder else generalized.columns.tolist()
        )

        return estimator.score(
            ArrayDataset(generalized_data, y_test, feature_names=feature_names)
        )
